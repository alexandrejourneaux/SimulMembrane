...
def cov_ref_vac(omega, detuning, input_transmission, phase_mm):
    ...
    cav = apply(t00 * FC(omega, detuning, input_transmission) + MM, cov_squeezed_inj_loss_vac)
    cav_losses = 1 - (abs(t00 * reflection(omega, detuning, input_transmission) + tmm)**2 + abs(t00 * reflection(-omega, detuning, input_transmission) + tmm)**2) / 2
    cav_vac = cav_losses * vacuum_cov
...
def cov_ref_ifo_vac(omega, detuning, input_transmission, phase_mm):
    '''covariance matrix after the interferometer'''
    
    return apply(IFO(omega), cov_ref_lossy_vac(omega, detuning, input_transmission, phase_mm))
...
def cov_ref_ro_vac(omega, detuning, input_transmission, phase_mm):
    ...
    return readout_losses * vacuum_cov + (1 - readout_losses) * cov_ref_ifo_vac(omega, detuning, input_transmission, phase_mm)
